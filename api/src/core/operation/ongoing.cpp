...
// this works only as long as the order in operation_arr is the same as the order in operation::type
...
operation::type operation::ongoing(const std::filesystem::path& operation_dir, char** error_message) {
    if (!std::filesystem::is_directory(operation_dir))
        return operation::type::none;
    for (int i = 0; operation_arr[i] != nullptr; i++)
        if (std::filesystem::is_regular_file(operation_dir / operation_arr[i]))
