...
static void get_all_paths(const std::filesystem::path& object_folder, std::vector<object::info>& out, const std::string tree_id, const std::filesystem::path current, std::unordered_set<std::string_view>& ignore, char** error_message) {
...
std::vector<object::info> version::all_objects(std::filesystem::path object_dir, std::string id, std::unordered_set<std::string_view> ignore, char** error_message) {
    if (id.empty())
        return {};
