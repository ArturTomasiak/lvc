...
static LvcVersion* history(LvcVersion*& allocated, const std::filesystem::path& object_dir, const std::filesystem::path& workspace_dir, const std::string& workspace_name, size_t depth, size_t length, size_t& index, char** error_message) {
    ...
    while (std::getline(file, line)) {
        ...
        if (depth && version_content.size() -1 >= VERSION_WORKSPACE)
            version->nested_versions = history(allocated, object_dir, workspace_dir, version_content[VERSION_WORKSPACE], depth - 1, length, index, error_message);
    }

    return first_version;
}

LvcVersion* version::history(const std::filesystem::path object_dir, const std::filesystem::path workspace_dir, const std::string workspace_name, size_t depth, size_t length, char** error_message) {
...
static LvcVersion* history_all(LvcVersion*& allocated, const std::filesystem::path& object_dir, const std::filesystem::path& workspace_dir, const std::string& workspace_name, size_t& allocated_size, size_t& index, char** error_message) {
    std::filesystem::path workspace = workspace_path(workspace_dir, workspace_name);
    if (!std::filesystem::exists(workspace))
        return {};
    std::ifstream file(workspace, std::ios::binary);
    ...
    while (std::getline(file, line)) {
        ...
        if (version_content.size() >= VERSION_WORKSPACE)
            version->nested_versions = history_all(allocated, object_dir, workspace_dir, version_content[VERSION_WORKSPACE], allocated_size, index, error_message);
    }

    return first_version;
}

LvcVersion* version::history_all(const std::filesystem::path object_dir, const std::filesystem::path workspace_dir, const std::string workspace_name, char** error_message) {
