...
static LvcVersion* history(LvcVersion*& allocated, const std::filesystem::path& object_dir, const std::filesystem::path& workspace_dir, const std::string& workspace_name, size_t depth, size_t length, size_t& index) {
    ...
    while (std::getline(file, line)) {
        ...
        if (depth && version_content.size() -1 >= VERSION_WORKSPACE)
            version->nested_versions = history(allocated, object_dir, workspace_dir, version_content[VERSION_WORKSPACE], depth - 1, length, index);
    }

    return first_version;
}

LvcVersion* version::history(const std::filesystem::path object_dir, const std::filesystem::path workspace_dir, const std::string workspace_name, size_t depth, size_t length) {
    LvcVersion* allocated = (LvcVersion*)malloc(length * sizeof(LvcVersion));
    size_t index = 0;
    return history(allocated, object_dir, workspace_dir, workspace_name, depth, length, index);
}

static LvcVersion* history_all(LvcVersion*& allocated, const std::filesystem::path& object_dir, const std::filesystem::path& workspace_dir, const std::string& workspace_name, size_t& allocated_size, size_t& index) {
    std::filesystem::path workspace = workspace_path(workspace_dir, workspace_name);
    if (!std::filesystem::exists(workspace))
        return {};
    std::ifstream file(workspace, std::ios::binary);
    ...
    while (std::getline(file, line)) {
        ...
        if (version_content.size() >= VERSION_WORKSPACE)
            version->nested_versions = history_all(allocated, object_dir, workspace_dir, version_content[VERSION_WORKSPACE], allocated_size, index);
    }

    return first_version;
}

LvcVersion* version::history_all(const std::filesystem::path object_dir, const std::filesystem::path workspace_dir, const std::string workspace_name) {
    size_t allocated_size = 50;
    size_t index          = 0;
    LvcVersion* allocated = (LvcVersion*)malloc(allocated_size * sizeof(LvcVersion));
    return history_all(allocated, object_dir, workspace_dir, workspace_name, allocated_size, index);
}
